...
attacked, and a target difficulty corresponding to the amount of connections the
...
transaction ID of the PAY\_TO\_DDOS transaction is committed to in
...
could indicate to the bounty provider that the target is claiming its bounty.
...
To evaluate the performance and effectiveness of DDoSCoin, we implemented our
...
transactions similar to Bitcoin, trust-less (atomic) cross-chain transactions can allow
...
Furthermore, we are not publishing a working alt-coin that uses this proof-of-DDoS,
but rather a conceptual description of one.
We believe it is important to fully disclose potential attacks, even those that
...
The authors are grateful for the support and various discussions from our
colleagues on DDoSCoin. In particular, the authors wish to thank David Adrian,
Matt Bernhard, and Drew Springall for their discussions concerning the
feasibility of the proof-of-DDoS idea. We also thank the anonymous reviewers
